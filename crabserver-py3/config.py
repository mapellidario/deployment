...
srv.thread_pool = 25
main.application = 'crabserver-py3'
main.port = 8270
main.index = 'data'
...
app = conf.section_('crabserver-py3')
...
data.serverhostcert = "%s/auth/crabserver-py3/dmwm-service-cert.pem" % __file__.rsplit('/', 3)[0]
data.serverhostkey = "%s/auth/crabserver-py3/dmwm-service-key.pem" % __file__.rsplit('/', 3)[0]
data.credpath = '%s/state/crabserver-py3/proxy/' % __file__.rsplit('/', 4)[0]
...
data.loggingFile = '%s/logs/crabserver-py3/CRAB-%s.log' % (__file__.rsplit('/', 4)[0], time.strftime("%Y%m%d"))
