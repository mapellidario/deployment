...
data.serverhostcert = "%s/auth/crabrest/dmwm-service-cert.pem" % __file__.rsplit('/', 3)[0]
data.serverhostkey = "%s/auth/crabrest/dmwm-service-key.pem" % __file__.rsplit('/', 3)[0]
data.credpath = '%s/state/crabrest/proxy/' % __file__.rsplit('/', 4)[0]
...
data.loggingFile = '%s/logs/crabrest/CRAB-%s.log' % (__file__.rsplit('/', 4)[0], time.strftime("%Y%m%d"))
